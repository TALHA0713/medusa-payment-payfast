...
import {
  ...
  PayFastS2SResponseData,
} from "../../types";
import PayFastProviderService from "../../services/payfast-provider";

const PAYMENT_PROVIDER_KEY = "pp_payfast";
...
export function constructWebhook({
  ...
  const PayFastProviderService = container.resolve(
    PAYMENT_PROVIDER_KEY
  ) as PayFastProviderService;

  logger.info(
    `signature ${signature}\n encoded: ${JSON.stringify(encodedBody)}`
  );
  return PayFastProviderService.constructWebhookEvent(
...
export function buildError(
  ...
  let message = `PayFast webhook ${event} handling failed${EOL}${
    err?.code ?? err?.message
  }`;
  if (err?.code === PostgresError.SERIALIZATION_FAILURE) {
    message = `PayFast webhook ${event} handle failed. This can happen when this webhook is triggered during a cart completion and can be ignored. This event should be retried automatically.${EOL}${
      err?.detail ?? err?.message
    }`;
  }
  if (err?.code === "409") {
    message = `PayFast webhook ${event} handle failed.${EOL}${
      err?.detail ?? err?.message
    }`;
  }
...
export async function handlePaymentHook({
  ...
  event: PayFastEvent;
  container: AwilixContainer;
  paymentIntent: PayFastS2SResponse;
}): Promise<{ statusCode: number }> {
  const logger = container.resolve("logger") as Logger;
  // logger.info("Data received: " + JSON.stringify(paymentIntent));

  let cartId = paymentIntent.data.merchantTransactionId;
...
export function createPostCheckSumHeader(
  ...
  const SALT_KEY = salt ?? process.env.PAYFAST_SALT ?? "test-salt";
